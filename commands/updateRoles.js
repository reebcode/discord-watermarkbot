module.exports = {
    ...
    async execute(message, args, Discord) {
        ...
        message.guild.members.cache.forEach(member => {
            if ((member.roles.cache.has(message.mentions.roles.first().id) || higherRole(message.mentions.roles.first().id, member) != undefined)) {
                console.log(member.user.username.toLowerCase().split(" ").join("-") + "has higher.");
            }
        })
